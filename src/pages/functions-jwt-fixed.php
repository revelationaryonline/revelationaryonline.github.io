...
// Add support for JWT Authorization header format
add_filter('determine_current_user', 'jwt_authenticate_user', 10);
function jwt_authenticate_user($user) {
    ...
    if (strpos($auth_header, 'JWT ') !== 0) {
        return $user;
    }
    
    // Extract token from header
    $token = substr($auth_header, 4);
    if (!$token) {
        return $user;
    }
    
    // JWT Authentication plugin should handle the rest
    // This hook is just to ensure we're checking for 'JWT ' prefix
    // in addition to the default 'Bearer ' prefix
